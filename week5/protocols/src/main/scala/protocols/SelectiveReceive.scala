...
object SelectiveReceive:
  ...
  def apply[T: ClassTag](bufferCapacity: Int, initialBehavior: Behavior[T]): Behavior[T] = {
    if (bufferCapacity == 0) { // no buffering
      Behaviors.same
    }

    try {
      Behavior.validateAsInitial(initialBehavior)
      Behaviors.withStash(bufferCapacity)(buffer => intercept(bufferCapacity, buffer, initialBehavior))
    } catch {
  ...
  private def intercept[T: ClassTag](bufferSize: Int, buffer: StashBuffer[T], started: Behavior[T]): Behavior[T] =
    Behaviors.receive { case (ctx, message) =>
      ...
      val isUnhandled   = Behavior.isUnhandled(nextBehavior)

      if (isUnhandled) {
        buffer.stash(message)
        Behaviors.same
      } else {
        val selectiveReceive = SelectiveReceive(bufferSize, Behavior.canonicalize(nextBehavior, started, ctx))
        buffer.unstashAll(selectiveReceive)
      }
    }
