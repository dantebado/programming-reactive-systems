...
object Transactor:
  ...
  def apply[T](value: T, sessionTimeout: FiniteDuration): Behavior[Command[T]] =
    SelectiveReceive(capacity, idle[T](value, sessionTimeout).narrow)
  ...
  /**
    ...
    */
  private def inSession[T](rollbackValue: T, sessionTimeout: FiniteDuration, sessionRef: ActorRef[Session[T]]): Behavior[PrivateCommand[T]] = Behaviors.receivePartial {
    case (_, Committed(session, newValue)) => {
