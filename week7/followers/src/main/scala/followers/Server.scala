...
object Server extends ServerModuleInterface:
  ...
  val reintroduceOrdering: Flow[Event, Event, NotUsed] =
    Flow[Event].statefulMapConcat(() => {
      ...
      @tailrec
      def getSeqNumber(nextSeqNumber: Int, orderedSequence: Queue[Event]): Queue[Event] =
        if (buffer.contains(nextSeqNumber)) {
          val nextEvent = buffer(nextSeqNumber)
          buffer -= nextSeqNumber
          seqNumber = nextSeqNumber + 1
          getSeqNumber(nextSeqNumber + 1, orderedSequence.enqueue(nextEvent))
        } else {
          orderedSequence
        }

      event => {
        buffer += (event.sequenceNr -> event)
        getSeqNumber(seqNumber, Queue.empty)
      }
    })
  ...
  val followersFlow: Flow[Event, (Event, Followers), NotUsed] =
    Flow[Event].statefulMapConcat(() => {
      var followers: Followers = Map().withDefault(_ => Set())

      {
        case event @ Follow(_, follower, followee) =>
          followers += (follower -> (followers(follower) + followee))
          (event, followers) :: Nil
        case event @ Unfollow(_, follower, followee) =>
          followers += (follower -> (followers(follower) - followee))
          (event, followers) :: Nil
        case event =>
          (event, followers) :: Nil
  ...
  def isNotified(userId: Int)(eventAndFollowers: (Event, Followers)): Boolean = eventAndFollowers._1 match {
    case Follow(_, _, followee)       => followee == userId
    case Unfollow(_, _, _)            => false
    case Broadcast(_)                 => true
    case PrivateMsg(_, _, recipient)  => recipient == userId
    case StatusUpdate(_, followee)    => eventAndFollowers._2.getOrElse(userId, Set()).contains(followee)
  }
...
class Server(using ExecutionContext, Materializer)
  ...
  def clientFlow(): Flow[ByteString, ByteString, NotUsed] =
    ...
    val sink: Sink[ByteString, NotUsed] =
      identityParserSink.mapMaterializedValue(v => {
        v.onComplete(clientIdPromise.complete)
        NotUsed
      })

    val source = Source.futureSource(clientIdPromise.future.map { identity =>
